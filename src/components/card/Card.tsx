...
function Card({ icon, title, statsNumber }: CardProps) {
  return (
    <article
      className={`flex flex-col group gap-5 bg-muted px-6 py-4 grow min-w-[15rem] lg:max-w-[20rem] rounded-3xl hover:bg-foreground hover:text-background transition-all duration-300`}
    >
      {/* top */}
      ...
      <div className="bottom flex flex-col gap-2">
        <h3 className="text-sm text-muted-foreground group-hover:text-gray-200">{title}</h3>
